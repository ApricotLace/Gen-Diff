...
const renderActions = [
  {
    ...
    makeDiff: (node, spaces, fn, updDepth) => `${' '.repeat(spaces)}${node.key}: ${fn(node.childrens, updDepth)}`,
  },
  {
    check: node => node.stage === 'unchanged',
    makeDiff: (node, spaces) => makeDiffSubStr(spaces, ' ', node.key, node.value),
  },
  {
    check: node => node.stage === 'changed',
    makeDiff: (node, spaces) => [
...
const render = (ast, currentDepth = 1) => {
  ...
  const diff = ast.map((node) => {
    ...
    return makeDiff(node, spaces, render, updatedDepthValue);
  }).flat().join('\n');
